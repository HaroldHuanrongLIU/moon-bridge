...
默认主配置文件为 `${XDG_CONFIG_HOME:-$HOME/.config}/moonbridge/config.yml`。扩展配置统一写在主配置的顶层 `extensions:` 下；扩展自己的参数放在 `config:`，启用开关可按全局、provider、model、route 四层覆盖：
...
模型或路由启用扩展时使用同一个 `extensions.<name>.enabled` 插槽，避免把扩展字段硬编码进核心配置结构。
...
Visual 是给 Anthropic-routed 主模型注入视觉能力的转发层扩展。主模型仍走原本 Provider，Visual 工具调用会通过 `extensions.visual.config.provider` 指定的现有 Anthropic-compatible Provider 执行，例如 Kimi；Moon Bridge 不引入单独的 Kimi 客户端或自定义桥接。

启用方式分两层：顶层 `extensions.visual.config` 声明视觉 Provider 和参数，具体主模型用 `extensions.visual.enabled: true` 暴露两个工具：
...
在具体 Provider 模型或 route 下设置 `extensions.deepseek_v4.enabled: true` 可启用 DeepSeek V4 专用兼容逻辑，包括 reasoning_content 剥离与重注入、thinking 回放、推理输出展示等。推理档位使用与 Codex 兼容的 `default_reasoning_level` / `supported_reasoning_levels` 元数据表达；Transform 会把请求里的 `reasoning.effort` 映射到 DeepSeek Anthropic 兼容参数 `output_config.effort`，其中 `xhigh` 会映射为 DeepSeek 的 `max`。流式 signature-only thinking 会编码进 Codex 可回放的 `reasoning.summary`，旧历史缺少 reasoning/缓存时只在请求侧补空 `thinking` block，不生成空 summary。详见 [docs/deepseek-v4.md](docs/deepseek-v4.md)。
    ...
    deepseek:          # 提供商标识符（自定义）
      ...
      models:
        deepseek-v4-pro:
          ...
          extensions:
            deepseek_v4:
              enabled: true         # 启用 DeepSeek V4 扩展
          default_reasoning_level: "high"
          supported_reasoning_levels:
            - effort: "high"
              description: "High reasoning effort"
...
模型标记 `extensions.deepseek_v4.enabled: true` 后，插件会自动处理：
...
在顶层 `extensions` 节下配置各扩展的参数；扩展自己的字段放在 `config` 中，启用状态放在对应 scope 的 `enabled` 中：
...
可读日志和 session 统计使用 Provider 计费维度：首次输入、缓存写入、缓存读取和输出分别累计与计价。Anthropic 路径直接使用上游 raw usage；OpenAI Responses 路径从 `usage.input_tokens - input_tokens_details.cached_tokens` 得到首次输入。Response normalized usage 只用于下游 OpenAI Responses 客户端兼容。

`metrics` 扩展的 `/v1/admin/metrics` 记录 Provider 原始 telemetry，和 Response normalized 不是同一层口径：`raw_*` 字段保留上游原始 usage（如 Anthropic 的 `input_tokens`、`cache_creation_input_tokens`、`cache_read_input_tokens`，或 OpenAI Responses 的 `usage.input_tokens` / `input_tokens_details.cached_tokens`），`normalized_*` 和历史 `input_tokens` / `cache_read` 字段保留 Moon Bridge 输出给下游客户端的兼容口径。OpenAI Responses 原生 usage 不提供 cache creation，metrics 中 `raw_cache_creation` 会保持 0。
...
| `visual` | 视觉扩展。将图片分析委派给专门的视觉 Provider（如 Kimi、GPT-4o） |
