...
- 校验 `mode`、`log`（level/format）、`system_prompt`、多 Provider 必填字段（`providers.<key>.base_url` / `api_key`）、模型路由和缓存参数。
...
- web search 配置支持三级覆盖：route → model（provider catalog）→ provider → 全局。推荐在顶层 `models.<slug>.web_search` 按模型单独配置。
...
- Provider 通过 `providers.<key>.offers[].model` 引用顶层 `models` 中的共享模型定义，`offers[].pricing` 提供按提供商定价。
- `providers.<key>.protocol` 决定该 Provider 走 Anthropic 转换还是 OpenAI Responses 直通。
...
Moon Bridge 支持多提供商和多模型路由。配置通过顶层 `providers` 定义提供商，顶层 `models` 定义共享模型元数据，顶层 `routes` 定义路由别名。
...
- `providers.<key>.offers[].pricing` 提供按 M tokens 计价的 input/output/cache write/cache read 价格。
...
- 配置中通过顶层 `providers` 定义多个 Provider（DeepSeek、OpenAI、Anthropic 等）
- 共享模型定义在顶层 `models`，每个 Provider 通过 `offers` 声明提供的模型及定价
...
费用依据 `providers.<key>.offers[].pricing` 配置计算，价格单位是人民币元 / M tokens。价格定义在 Provider 的模型目录中，通过 `routes` 关联到别名后自动生效。
